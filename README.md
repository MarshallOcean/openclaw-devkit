...
>
> | 运行方式 | `gateway.bind` 要求 | 安全影响 |
> | :------- | :------------------ | :------- |
> | **Docker 容器** | `lan` (绑定 `0.0.0.0`) | ✅ 安全 — Docker 端口映射限制外部访问 |
> | **宿主机直运行** | `loopback` (绑定 `127.0.0.1`) | ⚠️ `lan` 模式会暴露到局域网 |
>
> ### 为什么？
> - **容器内**：即使服务绑定 `0.0.0.0`，Docker 端口映射 `127.0.0.1:18789:18789` 仍只允许本地访问
> - **宿主机直运行**：如果 `bind=lan`，服务会监听所有网卡，局域网内其他设备可直接访问
>
> ### 推荐做法
